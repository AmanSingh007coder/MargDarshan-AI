      ...
      <header className="border-b border-white/5 bg-[#050810] sticky top-0 z-50 backdrop-blur-xl">
        <div className="px-10 py-6 flex items-center justify-between">
          <div className="flex items-center gap-8">
            <img src="/logo.svg" alt="MargDarshan" className="h-14 object-contain" style={{ filter: 'drop-shadow(0 0 10px rgba(6,182,212,0.4))' }} onError={(e) => { e.target.style.display = 'none'; }} />
            <nav className="flex items-center gap-6 border-l border-white/5 pl-8">
              {navItems.map(item => (
                <Link key={item.path} to={item.path}
                  className={`flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.2em] transition-all py-2 px-3 rounded-lg ${location.pathname === item.path || location.pathname.startsWith(item.path + '/')
...
export default function App() {
  return (
    <Routes>
      ...
      <Route path="/dashboard/map/:shipmentId" element={<DashboardLayout><LiveMap /></DashboardLayout>} />
